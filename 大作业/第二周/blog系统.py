        ...
        if judge.isdigit() and int(judge) in (1, 2):
            if int(judge) == 1:
                ...
                if c['|'] < 1:
                    ...
                    targetDir = os.path.join(os.path.abspath(os.curdir), username)
                    print(targetDir)
                    if not os.path.exists(targetDir):
                        os.makedirs(targetDir)
                    lst1 = []
...
@auth
def dariy(username):
    print(f"欢迎{username}来到日记页面,再见！")
    time.sleep(1)


@auth
def collection(username):
    print(f"欢迎{username}来到收藏界面,再见！")
    time.sleep(1)


@auth
def comment(username):
    print(f"欢迎{username}来到评论界面,再见！")
    time.sleep(1)
...
: '''
        ...
        if judge.isdigit() and int(judge) in (1, 2, 3, 4, 5, 6, 7, 8):
            if int(judge) == 1:
                global uname
                uname = login()
